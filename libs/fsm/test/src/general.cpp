...
* \date   21.11.2006
* 
* \brief  General functionality tests
*/
...
namespace GeneralTest {
	...
	typedef fsm::state_machine< StatesList_t > StateMachine_t;

} // namespace GeneralTest

using namespace GeneralTest;


BOOST_AUTO_TEST_CASE(event_delivery)
{
	TEST_ENTER(event_delivery);
...
BOOST_AUTO_TEST_CASE(enter_leave_reset)
{
	TEST_ENTER(enter_leave_reset);
...
void my_unexpected_event_handler(boost::any const& evt, std::type_info const& state, fsm::state_id_t id)
{
	if (evt.type() != typeid(Event3< int >))
	{
		std::string err = "my_unexpected_event_handler: The event type is not Event3< int >: ";
		err += evt.type().name();
	...
	if (event.value != 10)
	{
		std::string err = "my_unexpected_event_handler: The event value is not 10: ";
	...
	{
		std::string err = "my_unexpected_event_handler: The current state is not InitialState: ";
		err += state.name();
		throw std::logic_error(err);
	}
	...
	{
		std::string err = "my_unexpected_event_handler: The current state id is not InitialState::state_id (";
...
BOOST_AUTO_TEST_CASE(unexpected_events_handling)
{
	TEST_ENTER(unexpected_events_handling);
	...
	fsm.set_unexpected_event_handler(&my_unexpected_event_handler);
	fsm.process(Event3< int >(10));
}

BOOST_AUTO_TEST_CASE(bad_state_ids_handling)
{
	TEST_ENTER(bad_state_ids_handling);
